    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-base-100/90 backdrop-blur-xl border-t border-base-content/5 pb-safe md:hidden">
            <div className="flex items-center justify-around h-12 px-2">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={({ isActive }) => isActive ? "currentColor" : "none"} stroke="currentColor" strokeWidth={2} className="w-4 h-4">
                ...
                <NavLink
                    ...
                    className={({ isActive }) => `p-1 rounded-full transition-all ${isActive ? "border-primary scale-110" : "border-transparent"}`}
                >
                    <Avatar src={currentUser?.imageUrl} alt={username} size="xs" className="w-4 h-4" />
                </NavLink>
            </div>
        </div>
    );
